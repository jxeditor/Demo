package com.test.flink.stream.cep
  ...
  override def processElement(value: LoginEvent, ctx: KeyedProcessFunction[Long, LoginEvent, Warning]#Context, out: Collector[Warning]): Unit = {
    ...
    if (value.eventType == "fail") {
      ...
      if (iter.hasNext) {
        ...
        if (value.eventTime < firstFail.eventTime + maxFailTimes) {
          // 如果两次间隔小于2秒，输出报警
          println(Warning(value.userId, firstFail.eventTime, value.eventTime, "login fail in " + maxFailTimes + " seconds."))
          out.collect(Warning(value.userId, firstFail.eventTime, value.eventTime, "login fail in " + maxFailTimes + " seconds."))
        }
