...
public class TableEnvironmentImpl implements TableEnvironmentInternal {
	...
	private TableResult executeOperation(Operation operation) {
		if (operation instanceof ModifyOperation) {
			PlannerQueryOperation child = (PlannerQueryOperation) ((ModifyOperation) operation).getChild();
			LogicalProject calciteTree = (LogicalProject)child.getCalciteTree();
			List<RexNode> childExps = calciteTree.getChildExps();
			for (RexNode childExp : childExps) {
				RexCall rexCall = (RexCall) childExp;
