...
public class OrcFileDemo {
    public static void main(String[] args) throws IOException {
        ...
        for (int r = 0; r < 1; ++r) {
            int row = batch.size++;
            platform.setVal(row, UUID.randomUUID().toString().getBytes());
            event.setVal(row, UUID.randomUUID().toString().getBytes());
            dt.setVal(row, UUID.randomUUID().toString().getBytes());
